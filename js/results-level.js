...
function jsonCallback(item) {
  return function(data) {
		...
		for (j = 0; j < len; j++) {
		  var line=data.shift()
		  document.getElementById('subjectListContainer').innerHTML=document.getElementById('subjectListContainer').innerHTML + '<li><a href="' + name + '/' + line.subject_name_clean.replace(/\W+/g, '-').toLowerCase() + '.php?v=20180823">' + line.subject_name_clean + '</a></li>'
		}
  };
}
