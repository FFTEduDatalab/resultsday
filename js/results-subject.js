var urlLevel,
	...
	levels = [
		{
			'name': 'A-Level',
			'subjectsJSON': 'a-level-subjects.json?v=20200813',
			'entriesJSON': 'a-level-entries.json?v=20200813',
			'gradesJSON': 'a-level-grades.json?v=20200813',
			'textJSON': 'a-level-text.json?v=20200813',
		...
		{
			'name': 'AS-Level',
			'subjectsJSON': 'as-level-subjects.json?v=20200813',
			'entriesJSON': 'as-level-entries.json?v=20200813',
			'gradesJSON': 'as-level-grades.json?v=20200813',
			'textJSON': 'as-level-text.json?v=20200813',
			'gradesAll': ['A', 'B or above', 'C or above', 'D or above', 'E or above', 'U or above'],
			'gradesSelected': ['A', 'C or above', 'E or above']
		},
		{
			'name': 'GCSE',
			'subjectsJSON': 'gcse-subjects.json?v=20200813',
			'entriesJSON': 'gcse-entries.json?v=20200813',
			'gradesJSON': 'gcse-grades.json?v=20200813',
			'textJSON': 'gcse-text.json?v=20200813',
									...
									if (relatedArray.length == array.length) {
										relatedArray.forEach(function (subject_name_clean, innerIndex, innerArray) {
											if (innerIndex != innerArray.length - 1) {		// final element
												$('#related-subjects-section h5')[0].innerHTML = $('#related-subjects-section h5')[0].innerHTML + ' <a href="/' + level.toLowerCase() + '/' + subject_name_clean.replace(/\W+/g, '-').toLowerCase() + '.php?v=20200813">' + subject_name_clean + '</a>,';
											} else {
												$('#related-subjects-section h5')[0].innerHTML = $('#related-subjects-section h5')[0].innerHTML + ' <a href="/' + level.toLowerCase() + '/' + subject_name_clean.replace(/\W+/g, '-').toLowerCase() + '.php?v=20200813">' + subject_name_clean + '</a>';
											}
										});
...
function goBack () {
	...
	if (urlLevel == 'a-level') {
		window.location.href = '/a-level.php?v=20200813';
	}
	else if (urlLevel == 'as-level') {
		window.location.href = '/as-level.php?v=20200813';
	}
	else if (urlLevel == 'gcse') {
		window.location.href = '/gcse.php?v=20200813';
	}
}
